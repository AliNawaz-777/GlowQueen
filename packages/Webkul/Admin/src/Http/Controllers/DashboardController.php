...
class DashboardController extends Controller
{
    ...
    {
       return $this->orderItemRepository->getModel()
            ->select(DB::raw('SUM(order_items.qty_ordered) as total_qty_ordered'))
            ->addSelect('order_items.id', 'order_items.product_id', 'order_items.product_type', 'order_items.name')
            ->leftJoin('products', 'products.id', 'order_items.product_id')
            ->where('order_items.created_at', '>=', $this->startDate)
            ->where('order_items.created_at', '<=', $this->endDate)
            ->groupBy('products.id')
            ->havingRaw('SUM(order_items.qty_ordered) > 0')
            ->orderBy('total_qty_ordered', 'ASC')
            ->limit(5)
            // ->latest()
            ->get();
    }
    ...
    public function getCustomerWithMostSales()
    {
        return $this->orderRepository->getModel()
            ...
            ->havingRaw('SUM(base_grand_total) > 0')
