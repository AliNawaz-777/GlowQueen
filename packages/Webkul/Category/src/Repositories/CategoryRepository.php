...

{
...

    {
        ...
        return 'Webkul\Category\Contracts\CategoryTranslation';
...

    {
        ...
        return CategoryTranslation::where('id', '=', $id)->get();
...

    {
        ...
        $collection_id = CategoryTranslation::where('name', 'Collections')->get();

        $id = [];

        foreach ($collection_id as $new_id) {
        ...
        $collections_ids = $this->model->where('parent_id', $id)->get();

        for ($i=0; $i < count($collections_ids); $i++) { 

            $collection_array[$i]['name'] = $collections_ids[$i]['name'];

            $collection_array[$i]['image'] = $collections_ids[$i]['image'];

            $collection_array[$i]['slug'] = $collections_ids[$i]['slug'];
