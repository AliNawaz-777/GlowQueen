 ...
 * @copyright 2020 Arhamsoft Pvt Ltd (https://www.arhamsoft.com)
 */
class Cart {
    ...
    public function addProduct($productId, $data)
    {
        ...
        if (is_string($cartProducts)) {
            ...
            foreach ($cartProducts as $cartProduct) {                
                //$slug = $this->productFlatRepository->getSlugByid($cartProduct['product_id']);
                //$cartProduct['url_key'] = $slug;                
    ...
    public function checkProductActive($product_id) {
        ...
        {
            $query->when('products.type!=configurable', function ($simpleProd, $value) 
            {
                $simpleProd->whereRaw('product_inventories.qty != 0 AND product_inventories.qty > (select count(*) from orders INNER JOIN order_items ON orders.id = order_items.order_id where order_items.product_id = products.id AND orders.status="pending")');
            })
            ->when('products.type=configurable', function ($configProd, $value) 
            {
                // $configProd->whereRaw('product_inventories.qty != 0 AND product_inventories.qty > (select count(*) from orders INNER JOIN order_items ON orders.id = order_items.order_id where order_items.product_id = products.id AND orders.status="pending")');
            })
