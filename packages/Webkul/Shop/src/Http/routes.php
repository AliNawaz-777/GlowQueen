...
Route::group(['middleware' => ['web', 'locale', 'theme', 'currency']], function () {
    ...
    // Get sale products
    Route::get('/sales_products', 'Webkul\Shop\Http\Controllers\HomeController@getSalesProducts')->name('home.sales.products');
    // Get new products
    Route::get('/new_products', 'Webkul\Shop\Http\Controllers\HomeController@getNewProducts')->name('home.new.products');
    // Get just for you products
    Route::get('/just_for_you_products', 'Webkul\Shop\Http\Controllers\HomeController@justForYouProducts')->name('home.justforyou.products');
    //Store front home
    ...
    Route::get('/shop', 'Webkul\Shop\Http\Controllers\ShopController@index')->defaults('_config', [
        'view' => 'shop::shop.index'
    ]);


    Route::get('/ajax-products', 'Webkul\Shop\Http\Controllers\HomeController@getShopCategoryProducts')->name('home.ajax.products');

    // Route for testing
    // Route::get('/demo', 'Webkul\Shop\Http\Controllers\HomeController@indexDemo')->defaults('_config', [
    //     'view' => 'shop::home.demo'
    // ])->name('shop.home.index');
    // comment routes 
    Route::post('/comment/save', 'Webkul\Shop\Http\Controllers\BlogController@saveComment');
    //subscription
    //subscribe
    ...
    Route::get('/demo/categories/{slug}', 'Webkul\Shop\Http\Controllers\CategoryController@index')->defaults('_config', [
        'view' => 'shop::products.index_test'
    ])->name('shop.categories.index_text');
    ...
    // testing add to cart
    //Cart Items Add
    Route::post('checkout/cartx/add/{id}', 'Webkul\Shop\Http\Controllers\CartController@add_testing')->defaults('_config', [
        'redirect' => 'shop.checkout.cart.index'
    ])->name('cart.add.demo');
    ...
    //Checkout get shipping Form Store
    Route::post('/checkout/get-shipping', 'Webkul\Shop\Http\Controllers\OnepageController@getShippingPage')->name('shop.checkout.get-shipping');

    //Checkout get shipping Form Store
    Route::post('/checkout/get-payment-page', 'Webkul\Shop\Http\Controllers\OnepageController@getPaymentPage')->name('shop.checkout.get-payment-page');
    ...
    Route::prefix('customer')->group(function () {
        ...
        // Auth Routes
        Route::get('wishlist/ajax-add/{id}', 'Webkul\Customer\Http\Controllers\WishlistController@addWishList')->name('customer.wishlist.ajax-add');
    ...
    Route::get('{slug}', 'Webkul\CMS\Http\Controllers\Shop\PagePresenterController@presenter')->name('shop.cms.page');

    Route::fallback('Webkul\Shop\Http\Controllers\HomeController@notFound');

    //Route::post('page/mail','Webkul\Customer\Http\Controllers\CustomerController@sendMail');

    Route::post('mail', 'Webkul\Customer\Http\Controllers\CustomerMail@sendMail');

    Route::get('blog/{slug}', 'Webkul\Shop\Http\Controllers\BlogController@blogDetails')->name('shop.blog.detail');
    Route::get('blog/previous-post/{slug}', 'Webkul\Shop\Http\Controllers\BlogController@previousPost')->name('shop.blog.previousPost');
});
